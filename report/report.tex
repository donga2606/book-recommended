...
The frontend (\texttt{Bookrecommendationsystemui/}) uses \textbf{React 18}, \textbf{Vite}, \textbf{React Router}, and \textbf{TanStack Query} \cite{react,vite,react_router,tanstack_query}. A shared \texttt{Layout} provides sidebar navigation, while authentication and role-based route guards control access to user/admin/data-scientist pages.
...
\subsection{API Response Mapping to Recommendation Page}

For each authenticated user, the backend returns a user-specific recommendation payload. The frontend Home page (\texttt{/}) reads this payload and renders each card directly from the response fields:
\begin{itemize}
    \item \texttt{book.title}, \texttt{book.author}, \texttt{book.cover\_url} $\rightarrow$ book card content
    \item \texttt{predicted\_rating} $\rightarrow$ ``Predicted rating'' badge/value
    \item \texttt{reason} $\rightarrow$ ``Why recommended'' explanation text
    \item \texttt{rank} $\rightarrow$ server-side ordering of top-$N$ items
\end{itemize}

A representative response from \texttt{GET /users/me/recommendations?limit=3} is:
\begin{small}
\begin{verbatim}
{
  "items": [
...
Thus, the recommendation page is personalized per user because the request is authenticated by JWT, and the server computes results from that user's rating history before returning the response.
