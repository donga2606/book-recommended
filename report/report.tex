...
Course evaluators / institution &
Assess design, methodology, and results &
This report, live demonstration \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{UC3 --- Admin catalog:} Admin creates or edits a book $\rightarrow$ \texttt{POST/PATCH /admin/books}.
...
Although the implementation is labeled ``SVD,'' training uses \textbf{latent factor matrix factorization} optimized with stochastic gradient descent (SGD), not a full singular value decomposition of $R$. The prediction function is:
\begin{equation}
...
For classification-style metrics, ratings are binarized at threshold $\tau = 3.5$: a book is ``liked'' if $r \geq \tau$. Predictions are positive if $\hat{r}_{ui} \geq \tau$. Standard definitions apply:
\begin{align}
...
The backend (\texttt{backend/app/}) is built with \textbf{FastAPI} and \textbf{SQLAlchemy}. On startup, tables are created and \texttt{seed\_data()} loads books from \texttt{BX-Books.csv} if the database is empty.
...
The frontend (\texttt{Bookrecommendationsystemui/}) uses \textbf{React 18}, \textbf{Vite}, \textbf{React Router}, and \textbf{TanStack Query}. A shared \texttt{Layout} provides sidebar navigation and a role switcher for demo accounts.
...
Table~\ref{tab:metrics} reports metrics produced by \texttt{train\_and\_evaluate\_svd} on the held-out test set. Training completed in approximately \textbf{1.5 seconds} for this configuration.
...
\begin{itemize}
    ...
    \item \textbf{Data Scientist Dashboard} --- model overview, metrics cards (RMSE, accuracy, precision, recall, F1), experiment table, and training dialog wired to \texttt{POST /ml/train}.
\end{itemize}
...
GET/POST/PATCH/DELETE & \texttt{/admin/books} & Admin \\
