...
import { listMyRatings, upsertUserRating } from "../../lib/userApi";
...
export function Home() {
  ...
  const { data: persistedRatings = [] } = useQuery({
    queryKey: ["my-ratings"],
    queryFn: listMyRatings,
  });
  const persistedRatingsMap = persistedRatings.reduce<Record<number, number>>((accumulator, item) => {
    accumulator[item.bookId] = item.stars;
    return accumulator;
  }, {});
    ...
    onSuccess: async (savedRating) => {
      queryClient.setQueryData(
        ...
        (previous: Array<{ bookId: number; stars: number; updatedAt: string }> | undefined) => {
          const list = previous ?? [];
          const withoutBook = list.filter((item) => item.bookId !== savedRating.bookId);
  ...
  const handleRating = (bookId: number, rating: number, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setUserRatings((previous) => ({ ...previous, [bookId]: rating }));
    ratingMutation.mutate({ bookId, stars: rating });
  };
              ...
              <div className="mb-3">
                ...
                >
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        className={`w-5 h-5 transition-colors ${star <= (hoveredStars[book.id] || userRatings[book.id] || persistedRatingsMap[book.id] || 0)
